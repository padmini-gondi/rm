...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($name_err) && empty($riskrating_err) && empty($producttype_err) && empty($instrument_err) && empty($sector_err) && empty($region_err) && empty($country_err) && empty($currency_err) && empty($content_err)){
        // Prepare an update statement
        $sql = "UPDATE investments SET name=?, riskrating=?, producttype=?, instrument=?, sector=?, region=?, country=?, currency=?, content=? WHERE id=?";
 
        if($stmt = $mysqli->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("sssssssssi", $name, $riskrating, $producttype, $instrument, $sector, $region, $country, $currency, $content, $id);
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <p>Please edit the input values and submit to update the investment.</p>
